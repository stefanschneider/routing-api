...
var _ = Describe("DB", func() {
	...
	Describe("etcd", func() {
		...
		Describe("Tcp Mappings", func() {
			...
			Describe(".SaveTcpMapping", func() {
				It("Creates a mapping if none exist", func() {
					err := etcd.SaveTcpMapping(tcpMapping)
					Expect(err).NotTo(HaveOccurred())
			...
			Describe(".ReadTcpMappings", func() {
				It("Returns a empty list of routes", func() {
					tcpMappings, err := etcd.ReadTcpMappings()
					Expect(err).NotTo(HaveOccurred())
					Expect(tcpMappings).To(Equal([]db.TcpMapping{}))
				})
